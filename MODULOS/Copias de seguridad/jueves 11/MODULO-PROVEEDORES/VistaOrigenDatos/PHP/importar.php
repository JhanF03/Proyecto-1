...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    while (($data = fgetcsv($handle, 1000, ";")) !== FALSE) {
        ...
        if ($fechacausacion_dt === false) {
            $errorMessage = 'Formato de fecha inválido en fecha de causación: ' . $trimmedData[10];
        ...
        if ($fechaprogramacion_dt === false) {
            $errorMessage = 'Formato de fecha inválido en fecha de causación: ' . $trimmedData[11];
            header("Location: ../../OTROS/errorformatofecha.php" . urlencode($errorMessage));
            exit;
        ...
        if ($fechadebitoauto_dt === false) {
            $errorMessage = 'Formato de fecha inválido en fecha de causación: ' . $trimmedData[24];
            header("Location: ../../OTROS/errorformatofecha.php" . urlencode($errorMessage));
            exit;
        ...
        if ($fechapago_dt === false) {
            $errorMessage = 'Formato de fecha inválido en fecha de causación: ' . $trimmedData[38];
            header("Location: ../../OTROS/errorformatofecha.php" . urlencode($errorMessage));
            exit;
